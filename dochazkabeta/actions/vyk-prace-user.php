  ...
  echo "<table class=\"dochazka_table\"><tr class=\"dochazka_tr\"><th>Datum</th><th>Zakazka</th><th>Odpracováno</th></tr>";
    while($row = $result->fetch_assoc()) {
      ...
      if($dayofweek == 0){
       ...
       echo "<tr ><td class=\"dochazka_td\">".$datum."   ".$den_v_tydnu."</td><td class=\"dochazka_td zakazka\">".$row["jmeno_zakazky"]."</td><td class=\"dochazka_td\">".$cas."</td></tr>";
    }
    echo "</table>";
  } else {

    $error_code = "<br> 0 výsledků";
    $status = "error";
  }
  $conn->close();
