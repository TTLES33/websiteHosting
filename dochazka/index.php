...
<title> Docházka - Road Traffic </title>
...
//jestli je stránka zavolána editací docházky(tisk docházky(dochazka.php))

//$editstatus = $_POST["updatestatus"];

  echo 
  "<script> 
  var postedupdatestatus = \"".$_POST["updatestatus"]."\"; 
  var posteddatum = \"".$_POST["datum"]."\";
  var posteduzivatel = \"".$_POST["uzivatel"]."\";
  var postedcasod = \"".$_POST["cas_od"]."\";
  var postedcasdo = \"".$_POST["cas_do"]."\";
   </script>";
...
?>
<script>
    var uzivatel_var = "<?php echo $prihlaseny_uzivatel_id;?>";
    var perms = "admin";
</script>

<div class="center-content">
<div class="form">
<div class="tab_nadpis">Editace Docházky</div>
<form id="sssssss" onsubmit="index_status_checker(); return false;">
<table class="index_form_table">
  <tr>
  <td>Uzivatel</td>
  <td>
  <select class="uz_select" id="uzivatel" oninput="StatusToInsert(); getUzivatelVar(); loadZakazky(inserttype='selectchange');" name="uzivatele_vyber">
        <?php
        ...
        $sql = "SELECT * FROM uzivatele WHERE id_uzivatele<>$prihlaseny_uzivatel_id ORDER BY prijmeni";
        $result = $conn->query($sql);        
        echo "<option value=".$prihlaseny_uzivatel_id.">".$prihlaseny_uzivatel."</option>";
        if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
            echo "<option value=". $row["id_uzivatele"]."> ". $row["prijmeni"]." ". $row["jmeno"].", ".$row["titul"]." ".$row["titul_za"]." </option>";
          }
        } else {
          echo "<br> 0 results";
        }
...
<tr>
    <td>Datum</td>
    <td><input class="form_input" type="date" oninput="timeLeft(); StatusToInsert();fieldDelete()" id="datum" value="<?php echo date("Y-m-d"); ?>" name="datum" required></th>
</tr>

<tr >
  <td>Začátek práce</td>
  <script>var konecprace = "konecprace";</script>
  <td><input class="form_input" type="time"  oninput="timeLeft(konecprace); " name="cas_od" id="cas_od" required></td>
</tr>

<tr>
    <td>Konec práce</td>
  
    <td><input class="form_input" type="time" oninput="timeLeft(konecprace)" name="cas_do" id="cas_do" required></td> 
</tr>
<tr>
    <td></td>
    <td class="pauzanaobed" id="pauzanaobed"></td> 
</tr>
</table>
<br>


<table class="zakazky" id="zakazky">
<tr>
...
<input  class="form_button_add" name="submit" type="button" onclick=" zakazky_select_add();"  value="+">
</div>
<div id="result"></div>
<br><br>

<input id="form_insert_button" class="form_button" type="submit" name="submit"   value="Uložit">
...
<div class="dochazka-history" id="dochazka-history"> 



</div>
<div class="ActionStatus" id="ActionStatus"> 
...
<tr>
  <td>Jméno: <?php echo $prihlaseny_uzivatel;?></td>
</tr>

<tr>
  <td>Datum</td>
  <td><input class="form_input" type="date" oninput="timeLeft(); fieldDelete();" id="datum" value="<?php echo date("Y-m-d"); ?>" name="datum" required></th>
</tr>
...
<?php
$submitname = "submit_";
echo "<input   id=\"form_insert_button\" class=\"form_button\" type=\"submit\" name=\"submit\"   value=\"Uložit\">";
?>
...
echo "<meta http-equiv=\"refresh\" content=\"0; url='/dochazka/login/index.php?page=index'\" />";
