...
if ($result->num_rows > 0) {
    $status = "success";
    $output_tabulka = "".$output_tabulka."<table class=\"dochazka_table\"><tbody><tr class=\"dochazka_tr\"><th colspan=\"2\">Datum</th><th>Od</th><th>Do</th><th>Odpracováno</th></tr>";
    while($row = $result->fetch_assoc()) {
        ...
        $datum_fst = $row["datum"];  
         ...
         $output_tabulka = "".$output_tabulka."<tr ><td class=\"dochazka_td\">".$datum."</td>   <td class=\"dochazka_td\">".$den_v_tydnu."</td><td class=\"dochazka_td\">".$cas_od_string."</td><td class=\"dochazka_td\">".$cas_do_string."</td><td class=\"dochazka_td\">".$odpracovano_hodiny.":".$odpravovano_minuty."</td></tr>";
    }
    $output_tabulka = "".$output_tabulka."</tbody>";
  } else {
    $status = "error";
  ...
  $conn->close();
        ...
        $output_tabulka = "".$output_tabulka."<tr style=\"height: 10px;\"><td></td><td></td><td></td><td></td></tr>";
        $output_tabulka = "".$output_tabulka."<tfoot class=\"testik\"><tr><th>". $dochazka_pocet." dnů</th><td></td><td></td>
        <th style=\"text-align: center;\">". $odpracovano_hodiny.":". $odpravovano_minuty." hod</th><td></td></tr></tfoot></table>";
